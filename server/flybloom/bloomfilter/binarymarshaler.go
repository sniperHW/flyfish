...
func (f *Filter) marshalZip() (buf *bytes.Buffer,
	hash [sha512.Size384]byte,
	err error,
) {
	//f.lock.RLock()
	...
	err = binary.Write(buf, binary.LittleEndian, uint64(len(zipOut)))
	if err != nil {
		return nil, hash, err
	}

	err = binary.Write(buf, binary.LittleEndian, zipOut)
	if err != nil {
		return nil, hash, err
	}
...
// MarshalBinary converts a Filter into []bytes
func (f *Filter) MarshalBinaryZip() (data []byte, err error) {
	buf, hash, err := f.marshalZip()
	if err != nil {
		return nil, err
	}

	debug(
		"bloomfilter.MarshalZipBinary: Successfully wrote %d byte(s), sha384 %v",
		buf.Len(), hash,
	)
	data = buf.Bytes()
	return data, nil
